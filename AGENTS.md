...
- **`IntegrationConfig` direct entity access from the browser.** Closed by task #30 (`getIntegrationConfigRedacted` + `updateIntegrationConfigSettings` + `refreshSOSToken` admin functions). Going forward: any code that calls `base44.entities.IntegrationConfig.filter/update/create` from a browser-side surface (`src/pages/**`, `src/components/**`) is a P0 finding. Use the gateway functions; tokens travel browser→server one-way and never come back. Server-side function code (`base44/functions/**/entry.ts`) using `asServiceRole.entities.IntegrationConfig` is fine — that's where tokens belong.
- **IntegrationConfig response containing raw secrets.** Any function that returns `access_token`, `refresh_token`, or `client_secret` as raw values in its response body is a P0. The redacted shape is locked: `has_*` booleans + `client_id` (not a secret) + `access_token_last4` ("...XXXX") + `token_expires_at` + `notes`. See `getIntegrationConfigRedacted` for the canonical shape.
...
- **`Project` SOS-owned field rep-writes.** Closed by tasks #32 (lifecycle fields declared in schema) + #91 (`writeProjectAsOwner` gateway + admin-only RLS) + #94 (`writeTapeRunAsOwner`). Going forward: any rep-context code path that calls `base44.entities.Project.update` or `base44.entities.TapeRun.update/create/delete` directly from the browser is a P0. Use the gateway functions (`writeProjectAsOwner`, `writeTapeRunAsOwner`) — they enforce ownership + field allowlists. Admin direct entity writes via the Builder remain allowed.
